...
CREATE TYPE TypeUtilisateur AS ENUM ('Administrateur', 'Recruteur', 'Candidat');
CREATE TYPE StatutCompte AS ENUM ('actif', 'inactif', 'bannis');
...
CREATE TABLE Utilisateur (
  ...
  statutCompte StatutCompte NOT NULL,
  mdpHash VARCHAR(255) NOT NULL,
  organisation VARCHAR(255),
  PRIMARY KEY (id),
  FOREIGN KEY (organisation) REFERENCES Organisation(siren),
  CHECK (typeUtilisateur IN ('Administrateur', 'Recruteur', 'Candidat')),
  CHECK (statutCompte IN ('actif', 'inactif', 'bannis')),
